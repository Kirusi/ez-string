...
break;
case 3:this.begin('expr'); return 7;
break;
...
case 10:return '\]';
break;
case 11:this.begin('string'); return 16;
break;
case 12:this.popState(); return 16;
break;
case 13:return 18;
break;
case 14:yy_.yytext = '\\'; return 18;
break;
case 15:yy_.yytext = '"'; return 18;
break;
case 16:return 18;
break;
case 17:return 5;
...
rules: [/^(?:(?:[^\{\}]|\n|[\(\)])+)/,/^(?:{{)/,/^(?:}})/,/^(?:\{)/,/^(?:\})/,/^(?:\s+)/,/^(?:([0-9]+))/,/^(?:[\$\_A-Za-z][\$\_A-Za-z0-9]*)/,/^(?:\.)/,/^(?:\[)/,/^(?:\])/,/^(?:\")/,/^(?:\")/,/^(?:(:?[^"(\\)]|[\(\)])+)/,/^(?:(\\)(\\))/,/^(?:(\\)")/,/^(?:(\\).)/,/^(?:$)/],
conditions: {"expr":{"rules":[4,5,6,7,8,9,10,11],"inclusive":false},"string":{"rules":[12,13,14,15,16],"inclusive":false},"INITIAL":{"rules":[0,1,2,3,17],"inclusive":true}}
});
return lexer;
})();
parser.lexer = lexer;
